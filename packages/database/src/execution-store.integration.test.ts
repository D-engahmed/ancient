...
describe.skipIf(!hasDb)("PostgresExecutionStore (integration, requires DATABASE_URL)", () => {
    ...
    it("appends a gapless seq stream and projects it via applyEvent", async () => {
        ...
        const store = first!;
        const c0 = await store.appendEvent({ id: `${TEST_PREFIX}e0`, executionId: execId, userId: "user-1", type: "created", timestamp: new Date("2026-01-01T00:00:00.000Z"), payload: { teamId: "t1", teamName: "Alpha", task: "Build it" } });
