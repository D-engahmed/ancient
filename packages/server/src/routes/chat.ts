...
type ANCIENTUIMessage = UIMessage<ChatMessageMetadata, never, InferUITools<ToolContracts>>;

const submitSchema = z.object({
  id: z.string(),
  messages: z
    .array(
      z.custom<ANCIENTUIMessage>((value) => {
...
function hasPendingToolCalls(message: ANCIENTUIMessage) {
  ...
  .post(
    ...
    async (c) => {
      ...
      const previousMessages = Array.isArray(session.messages)
        ? (session.messages as unknown as ANCIENTUIMessage[])
        : [];
      ...
      const nextMessages = await validateUIMessages<ANCIENTUIMessage>({
      ...
      return result.toUIMessageStreamResponse<ANCIENTUIMessage>({
