// file: packages/server/src/lib/model-router.ts
// Smart model routing — the cost/token-efficiency layer. When enabled, simple
// prompts are answered by a configured FREE model (OpenRouter :free tiers, or
// a fully local Ollama/LM Studio/vLLM model — zero cost, on-device), and only
// complex work keeps the premium company model the user selected.
//
// The classifier is a transparent heuristic, not magic: it scores the prompt
// and mode, and the user can turn the whole thing off in settings.json.

import type { ChatModelSelection } from "@ANCIENT/shared";
import type { ModeType } from "@ANCIENT/shared";
import type { ModelRoutingSettings } from "../hooks/settings";
